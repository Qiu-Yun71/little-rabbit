...
const GetCheckInfo = async () => {
    const response = await GetCheckInfoAPI()
    checkInfo.value = response.result

    const item = response.result.userAddresses.find(item => item.isDefault === 0)
...
<template>
    ...
    <el-dialog v-model="showDialog" title="切换收货地址" width="30%" center>
        <div class="addressWrapper">
            <div class="text item" :class="{ active: item.id === activeAdress.id }" @click="showAddress(item)"
